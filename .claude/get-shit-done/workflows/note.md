...
- **Global scope**: `/Users/axelignis/crisol/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/axelignis/crisol/.claude/notes/`)
...
2. Glob `/Users/axelignis/crisol/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/axelignis/crisol/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/axelignis/crisol/.claude/notes/` — works in any directory
