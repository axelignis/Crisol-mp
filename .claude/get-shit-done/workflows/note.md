...
- **Global scope**: `$HOME/crisol/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `$HOME/crisol/.claude/notes/`)
...
2. Glob `$HOME/crisol/.claude/notes/*.md` (if directory exists) — global notes
...
Global ($HOME/crisol/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `$HOME/crisol/.claude/notes/` — works in any directory
